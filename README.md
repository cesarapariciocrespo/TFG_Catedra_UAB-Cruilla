<p align="center">
  <img src="assets/portada-tfg.jpeg" alt="Portada del proyecto" width="800">
</p>

# Cátedra UAB-Cruïlla: espacios de experimentación e innovación en eventos públicos

Este repositorio reúne materiales complementarios del Trabajo de Fin de Grado **“Espacios de experimentación e innovación en eventos públicos: el caso de la Cátedra UAB-Cruïlla”**, realizado para el Grado en Gestión de Ciudades Inteligentes y Sostenibles de la Escola d’Enginyeria de la Universitat Autònoma de Barcelona.

El trabajo analiza las acciones impulsadas en el marco de la **Cátedra UAB-Cruïlla**, una colaboración entre la Universitat Autònoma de Barcelona y el Festival Cruïlla orientada a explorar el papel de la inteligencia artificial en la creación artística, la innovación cultural y la transferencia de conocimiento. El estudio presta especial atención a dos contextos de experimentación: la **Setmana de la Innovació de la UAB**, entendida como un entorno de Living Lab universitario, y el **Festival Cruïlla**, interpretado como una ciudad efímera donde los prototipos tecnológicos entran en contacto con públicos más amplios y heterogeneos.

Para más información sobre esta Cátedra, se puede consultar su página web en el siguiente enlace (https://catedrauabcruilla.cvc.uab.es/conference-artificial-intelligence-arts-and-innovation/).

## Objetivo del repositorio

El objetivo de este repositorio es facilitar el acceso a materiales adicionales que complementan el trabajo académico, incluyendo anexos, entrevistas, documentación visual, transcripciones, cuestionarios y otros recursos relacionados con el proceso de investigación, necesarios para la realización de este TFG.

## Contenido

- `paper/`: versión final del trabajo (en castellano e inglés).
- `transcripts/`: documentos adicionales, incluyendo entrevistas a los principales stakeholders y performaforums.
- `data/`: analisis complementarios al trabajo principal.
- `images/`: documentación visual de los espacios, prototipos y actividades analizadas.

## Resumen del trabajo

La investigación parte de la hipótesis de que determinados espacios culturales y artísticos pueden funcionar como infraestructuras de experimentación e investigación. A través del caso de la Cátedra UAB-Cruïlla, el trabajo estudia cómo la interacción entre universidad, festival, artistas, investigadores, estudiantes y público permite activar procesos de innovación aplicada en contextos reales.

En la Setmana de la Innovació, el campus universitario actúa como un Living Lab en el que los prototipos se presentan, discuten y validan en un entorno académico y experimental. En el Festival Cruïlla, en cambio, la investigación se desplaza hacia una ciudad efímera, caracterizada por una mayor diversidad de públicos, una interacción más espontánea y unas condiciones de validación más cercanas al uso social real de las tecnologías.

## Autor

**César Aparicio Crespo**  
Trabajo de Fin de Grado en Gestión de Ciudades Inteligentes y Sostenibles  
Escola d’Enginyeria, Universitat Autònoma de Barcelona  
Curso 2025/2026
